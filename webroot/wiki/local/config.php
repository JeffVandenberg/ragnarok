...
$WikiTitle = 'Dominium Fuego';

##  $ScriptUrl is your preferred URL for accessing wiki pages
##  $PubDirUrl is the URL for the pub directory.
$ScriptUrl = ('http' . (isset($_SERVER['HTTPS']) ? 's' : '')) . '://' . $_SERVER['SERVER_NAME'] . '/wiki';
$PubDirUrl = ('http' . (isset($_SERVER['HTTPS']) ? 's' : '')) . '://' . $_SERVER['SERVER_NAME'] . '/wiki/pub';
...
$DefaultPasswords['admin'] = crypt('r@gnar0k');
...
if(isset($_SESSION['permissions'])) {
   $permissions = $_SESSION['permissions'];
    ...
    $sql = "SELECT permission_id FROM  permissions_users where user_id = " . $userdata['user_id'];
    $result = $db->sql_query($sql);
    $permissions = [];
    $permissions['logged_in'] = $userdata['user_id'] != 1;
    while ($row = $db->sql_fetchrow($result)) {
        $permissions[$row['permission_id']] = true;
    }
    $_SESSION['permissions'] = $permissions;
}
$mayEdit = isset($permissions[6]);

$isLoggedIn = $userdata['user_id'] != 1;
if (isset($_GET['action'])) {
    if (($_GET['action'] == 'edit') || ($_GET['action'] == 'attr')) {
        if (!$isLoggedIn || !$mayEdit) {
            Abort('You may not edit');
        }
    }
}
...
if ($permissions['logged_in']) {
    $toolmenu = '<li>
        ...
        <ul>
	<li><a href="https://calendar.google.com/calendar/embed?src=ragnaroknycgmstaff%40gmail.com" target="_blank">Calendar</a></li>
        <li><a href="/diceRolls/scene">Scene Roller</a></li>';

    $toolmenu .= '<li><a href="/characters">Your Characters</a>';
    // view users
    if ($permissions[3]) {
        $toolmenu .= '<li><a href="/users">User management</a></li>';
    }

    // GM Tools
    if ($permissions[5]) {
        $toolmenu .= '<li><a href="/gamemaster">GM Tools</a></li>';
    }

    // GM Tools
    if ($permissions[2]) {
        $toolmenu .= '<li><a href="/configuration">Game Configuration</a>';
    }

    $toolmenu .= '</li>';
}

$FmtPV['$ToolMenu'] = "getToolMenu()";
function getToolMenu()
{
    global $toolmenu;
    return $toolmenu;
}
...
$FmtPV['$CharacterMenu'] = "getCharacterMenu()";
