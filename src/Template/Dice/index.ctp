...
/* @var AppView $this */
$this->set('title_for_layout', 'Dice Roller');
?>
...
<script type="text/javascript">
    ...
    function updateDiceImage(dice, result) {
        if(result === -1) {
            $("#dice-" + dice).attr('src', '<?php echo $this->Url->build('/') . 'img/fate_magicminus.png';?>');
        }
        if(result === 0) {
            $("#dice-" + dice).attr('src', '<?php echo $this->Url->build('/') . 'img/fate_magicblank.png';?>');
        }
        if(result === 1) {
            $("#dice-" + dice).attr('src', '<?php echo $this->Url->build('/') . 'img/fate_magicplus.png';?>');
        }
    }

    $(function() {
        $("#roll-dice").click(function() {
            $("#dice-0").attr('src', '<?php echo $this->Url->build('/') . 'img/fate_magicblank.png';?>');
            $("#dice-1").attr('src', '<?php echo $this->Url->build('/') . 'img/fate_magicblank.png';?>');
            $("#dice-2").attr('src', '<?php echo $this->Url->build('/') . 'img/fate_magicblank.png';?>');
            $("#dice-3").attr('src', '<?php echo $this->Url->build('/') . 'img/fate_magicblank.png';?>');
